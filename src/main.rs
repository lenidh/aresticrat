...
use config::{BackupOptions, CommandSeq, Config, ForgetOptions, Location, Name};
...
fn forget(config: &Config, args: &ForgetArgs) -> Result<()> {
    ...
    for t in args.locations() {
        let set = m.entry(t.location().clone()).or_insert_with(HashSet::new);
